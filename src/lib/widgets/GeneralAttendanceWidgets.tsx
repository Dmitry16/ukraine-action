...
export const GeneralAttendanceTableWidget = () => {
  const theme = useTheme()
  const vertical = useSelector((state: any) => state.app.vertical)
  const verticalConfig = useConfig(vertical)

  const components = verticalConfig?.dashboard?.body?.generalAttendanceFeature?.GeneralAttendanceTableWidget.components || []
...
export default GeneralAttendanceTableWidget
